...
from fat.fat_bert_webq.webq_ppr import _read_facts, _get_subgraph
...
def create_example_from_jsonl(line, documents, question_seeds):
  """Creates an NQ example from a given line of JSON."""
  e = json.loads(line, object_pairs_hook=collections.OrderedDict)
  question_text = e['question']
  # question_entities = e['entities']
  question_entities = question_seeds[e["id"]]
  candidate_passages = e['passages']
  answers = e['answers']
  answer_entities = [x['kb_id'] for x in answers]
  ...
  for id, item in candidate_passages:
    document_text = documents[item['document_id']]['document']['text']
    document_entities = documents[item['document_id']]['document']['entities']
    title_text = documents[item['document_id']]['title']['text']
    title_entities = documents[item['document_id']]['title']['entities']

    context_entities = []
    context_text = "[ContextId=%d] " % (id) +"[Title] "
    context_answer_span = [-1, -1]
    context_answer_text = None

    context_offset = len(context_text.split(" "))
    context_text += str(title_text) + " [Doc] "
    for item in title_entities:
        entity_text = title_text.split(" ")[item['start']:item['end']]
        item['start'] += context_offset
        item['end'] += context_offset
        context_entities.append(item)
        if item['text'] in answer_entities:
            context_answer_span[0] = item['start']
            context_answer_span[1] = item['end']
            context_answer_text = entity_text
            example['has_correct_context'] = True

    context_offset = len(context_text.split(" "))
    context_text += str(document_text)
    for item in document_entities:
        entity_text = title_text.split(" ")[item['start']:item['end']]
        item['start'] += context_offset
        item['end'] += context_offset
        context_entities.append(item)
        if item['text'] in answer_entities and context_answer_span != [-1, -1]:
            context_answer_span[0] = item['start']
            context_answer_span[1] = item['end']
            context_answer_text = entity_text
            example['has_correct_context'] = True

    if context_answer_span != [-1, -1]:
      expected = context_text.split(" ")[context_answer_span[0]:context_answer_span[1]]
      ...
      assert expected == context_answer_text, (expected, context_answer_text)

    example['contexts'].append({"context_text": context_text,
                                "context_entities": context_entities,
                                "context_answer_span": context_answer_span,
                                "context_answer_text": context_answer_text})
  return example
...
def get_related_facts(question_id, question_entities, answer_entities,
                      question_embeddings, relation_embeddings):
    fact_file = os.path.join(
        FLAGS.subgraph_dir, question_id + ".nxhd")
    question_embedding = question_embeddings[question_id]

    if not os.path.exists(fact_file):
        print("fact file not found for %s" % question_id)
    ...
    for ee in question_entities:
        if ee in entity_map:
            seed_entities.append(entity_map[ee])
    for answer in answer_entities:
        if answer in entity_map:
            ans_entities.append(entity_map[answer])

    if not seed_entities:
        print("No seeds found for %s!" % question_id)
        extracted_tuples, extracted_ents = [], []
    elif adj_mat is None:
        print("No facts for %s!" % question_id)
        extracted_tuples, extracted_ents = [], []
    else:
...
def convert_examples_to_features(examples, tokenizer, is_training, output_fn, pretrain_file=None):
  ...
  for example in examples:
    ...
    features, stats = convert_single_example(example, tokenizer, is_training, pretrain_file)
    num_spans_to_ids[len(features)].append(example.qas_id)

    for feature in features:
      feature.example_index = example_index
      feature.unique_id = feature.example_index + feature.doc_index
      output_fn(feature)
...
# def read_facts(subgraph_dir, question_id, ):
#     fact_file = os.path.join(
#         subgraph_dir, question_id + ".nxhd")
#     if not os.path.exists(fact_file):
...
def convert_single_example(example, tokenizer):
    ...
    answer_entities = example['answer_entities']
    extracted_tuples, extracted_entities = get_related_facts(question_id, question_entities, answer_entities,
                                                             question_embeddings, relation_embeddings)

    # QUERY
    query_tokens = []
    query_tokens.append("[Q]")
    query_tokens.extend(tokenize(tokenizer, question))
    ...
    for (doc_index, doc) in enumerate(example['contexts']):
        doc_tokens = doc['context_text'].split(" ")
        context_answer_spans = doc['context_answer_span']
        ...
        tok_end_position = 0
        tok_start_position = orig_to_tok_index[context_answer_spans[0]]
        if context_answer_spans[1] < len(doc_tokens) - 1:
            tok_end_position = orig_to_tok_index[context_answer_spans[1] + 1] - 1
        else:
            tok_end_position = len(all_doc_tokens) - 1
        if tok_start_position > tok_end_position :
            print(context_answer_spans[0], context_answer_spans[1])
        ...
        if FLAGS.augment_facts:
            max_tokens_for_current_facts = max_tokens_for_doc - curr_doc_length
        ...
        text_only_input_mask = [0 if item == 0 else 1 for item in text_only_input_ids]
        padding = [0] * (FLAGS.max_seq_length - len(text_only_input_ids))
        text_only_input_ids.extend(padding)
        text_only_input_mask.extend(padding)
        ...
        contains_an_annotation = context_answer_spans != [-1,-1]
        ...
        feature = InputFeatures(
            unique_id=-1,
            example_index=-1,
            doc_span_index=doc_index,
            tokens=tokens,
            token_to_orig_map=token_to_orig_map,
            # token_is_max_context=token_is_max_context,
            text_only_input_ids=text_only_input_ids,
            text_only_input_mask=text_only_input_mask,
            input_ids=input_ids,
        ...
        feature_stats.append({'fact_recall_counter':fact_recall_counter, 'answers_reached':answers_reached,
                              'answer_recall_counter':answer_recall_counter, 'answer_entity_ids':answer_entity_ids})
...
class CreateTFExampleFn(object):
  ...
  def process(self, example, pretrain_file=None, fixed_train_list=None):
    ...
    input_features, stat_counts = convert_single_example(example, self.tokenizer, self.apr_obj,
    ...
    for idx, input_feature in enumerate(input_features):
      input_feature.example_index = str(example["id"])
      input_feature.unique_id = (input_feature.example_index + "_" + str(input_feature.doc_index))
      ...
      features["start_positions"] = create_int_feature([input_feature.start_position])
      features["end_positions"] = create_int_feature([input_feature.end_position])
      features["answer_types"] = create_int_feature([input_feature.answer_type])
...
class InputFeatures(object):
  ...
  def __init__(self,
    ...
    self.text_only_input_mask=text_only_input_mask
...
def model_fn_builder(bert_config, init_checkpoint, learning_rate,
  ...
  def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
    ...
    if mode == tf.estimator.ModeKeys.TRAIN:
      ...
      total_loss = (start_loss + end_loss) / 3.0
...
def input_fn_builder(input_file, seq_length, is_training, drop_remainder):
  ...
  name_to_features = {"unique_ids": tf.FixedLenFeature([], tf.int64),
                      ...
                      "text_only_input_ids": tf.FixedLenFeature([seq_length], tf.int64),
                      "text_only_mask": tf.FixedLenFeature([seq_length], tf.int64),
                      ...
                      "shortest_path_num_hops": tf.FixedLenFeature([], tf.int64)}
