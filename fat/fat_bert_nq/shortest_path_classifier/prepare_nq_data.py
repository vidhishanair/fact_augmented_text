...
from fat.fat_bert_nq.shortest_path_classifier import nq_data_utils
...
def main(_):
  ...
  if FLAGS.is_training:
    ...
    instances = []
    input_file = FLAGS.train_data_file
    print("Reading file %s", input_file)
    file_stats_counter = {}
    for example in nq_data_utils.get_nq_examples(input_file):
      stats_count = None
      instance, stats_count = creator_fn.process(example)
      instances.append(instance)
      instances_processed += 1

      if instances_processed % 100 == 0:
    ...
    output_file = FLAGS.train_output_file
